...
def run(prompt, others = [], stream = True): # yield
    ...
    import time
    time.sleep(0.1)
    log("WARN|[server] " + str(history))
    with open( dirnow + "/prompt", encoding = "utf-8") as c:
...
def unsplit(text, others = []):
    global llm, log, search_url, history
    if text.split("!debug clear").__len__() > 1:
        history = []
        return "[debug] clear history."

    runer = run(text, others, False)

    ends = runer['choices'][0]["message"]["content"]
    history = history + [{"role":"user", "content": text},{"role": "assistant", "content": ends}]
    print(history)
    return ends
